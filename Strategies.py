...
def rsi_divergence_sl_tp(data, freq, window_rsi, order_rsi, rsi_up, rsi_down, plot_data={2:[('RSI',['rsi_up','rsi_down'],'blue')]}):
    ...
    #calculating RSI with talib
    df['RSI']=ta.RSI(df['Close'], window_rsi)
   
    #divergence points
    bear_div, bull_div=find_divergence(df, order_rsi)
    
    #datetime indexes where divergence occurs
    bear_points=[(df.index[a[1]],-1) for a in bear_div]
    bull_points=[(df.index[a[1]],1) for a in bull_div]
    
    #list of tuples with datetime index and position
    pos_div=bear_points+bull_points
    
    #creating dataframe from list of tuples       
    pos_df=pd.DataFrame(pos_div, columns=['time','position']).set_index('time')
    
    #concat dataframes by index
    df=pd.concat([df, pos_df], axis=1)
    
    df['RSI_shifted']=df['RSI'].shift(1)       
    df['position']=df['position'].fillna(0)
    
    #closing position rule when RSI crosses down and up levels
    df['position']=np.where(((((df['RSI']>rsi_up) &(df['RSI_shifted']<rsi_up) & (df['position']==0)) | ((df['RSI']<rsi_down)&(df['RSI_shifted']>rsi_down))) & (df['position']==0)),2,df['position'])
    df['position']=df['position'].replace(0, method='ffill')
